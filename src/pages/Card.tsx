...
export default function Card() {
  ...
  const joinUrl = `${appUrl}/join/${user?.id}`;
  const pass2uUrl = `https://www.pass2u.net/p/${import.meta.env.VITE_PASS2U_MODEL_ID}`;
  ...
  // Initialize form once client loads
  useEffect(() => {
  ...
  const copyLink = () => {
    navigator.clipboard.writeText(pass2uUrl);
  ...
  // client is guaranteed non-null here by ProtectedRoute, but guard just in case
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm">
              ...
              <div ref={qrRef} className="flex justify-center mb-4">
                <div className="p-4 border border-gray-100 rounded-xl">
                  <QRCodeSVG value={pass2uUrl} size={180} fgColor="#1E3932" />
                </div>
              </div>
              ...
              <div className="mb-4">
                <label className="block text-sm font-medium text-ink mb-1.5">Lien Pass2U</label>
                <input
                  type="text"
                  readOnly
                  value={pass2uUrl}
